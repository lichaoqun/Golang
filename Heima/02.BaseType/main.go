...
大小写
	变量/常量名字的开头字母的大小写决定了名字在包外的可见性
命名规则
	如果一个名字的作用域比较大，生命周期也比较长，那么用长的名字将会更有意义。对于局部变量可以尽量使用短小的名字.
	go语言推荐使用驼峰式命名，而对于而像ASCII和HTML这样的缩略词则避免使用大小写混合的写法，它们可能被称为htmlEscape、HTMLEscape或escapeHTML，但不会是escapeHtml。
类型声明的关键字
	var : 可以创建一个特定类型的变量，然后给变量附加一个名字，并且可以设置变量的初始值, 不设置的话使用默认的值.
		var 变量名字 类型 = 表达式, 其中“类型”或“= 表达式”两个部分可以省略其中的一个。如果省略的是类型信息，那么将根据初始化表达式来推导变量的类型信息。如果初始化表达式被省略，那么将用零值初始化该变量。
	const : 声明变量类型
	func : 声明一个函数类型
	type : 声明一个自定义类型
变量
	...
	变量声明的几种方式
		...
		5. 多重赋值, 只有自动推导类型的声明/初始化的情况可以同时声明/初始化多个类型, 否则只能声明/初始化相同的类型;
			多重赋值中使用自动类型推导, 左边的变量中应该必须至少有一个是新声明的变量.
	Println 和 Printf
